...
data['presion'] = data['presion'].str.replace(' hPa', '').astype(int)
...
chartData = [{'time': date.strftime('%Y-%m-%d'), 'value': temp} for date, temp in zip(data['fecha'], data['presion'])]

# LIT ES LA RUTA ACTUAL DEL ARCHIVO ACTUAL 
current_directory = os.path.dirname(os.path.abspath(__file__))

# RUTA DE JS CONSIDERANDO EN ESTE CASO EL ARCHIVO JS
js_file_path = os.path.join(current_directory, 'graficoP.js')
